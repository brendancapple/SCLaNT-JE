...
def geminiTranslation(input, temp, topp):
...
def translateCorpus(japaneseSentences, TEMPERATURE, TOPP):
  ...
  for sentence in japaneseSentences:
    try:
      response = geminiTranslation(sentence, TEMPERATURE, TOPP)
  ...
  filename = 'geminiData' + str(TEMPERATURE).replace('.', '') + "-" + str(TOPP).replace('.', '') + '.csv'
